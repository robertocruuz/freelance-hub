...
BEGIN
  IF TG_OP = 'UPDATE' THEN
    IF NEW.column_id IS DISTINCT FROM OLD.column_id OR (NEW.status = 'done' AND OLD.status IS DISTINCT FROM 'done') THEN
      ...
      -- Notificar APENAS: dono da tarefa + usuários com compartilhamento DIRETO da tarefa
      FOR _recipient_id IN
        SELECT DISTINCT u_id FROM (
          -- Dono da tarefa
          SELECT NEW.user_id as u_id
          UNION
          -- Compartilhado diretamente com usuário (apenas tarefa, não quadro)
          ...
          UNION
          -- Compartilhado com a organização (apenas tarefa, não quadro)
          SELECT om.user_id 
          FROM shares s
