-- Fix notify_on_share trigger to handle 'pipeline' and 'lead' resource types safely
...
BEGIN
  ...
  ELSIF NEW.resource_type = 'pipeline' THEN
    _notification_title := _creator_name || ' compartilhou um pipeline de leads';
    _notification_message := 'Um pipeline de leads foi compartilhado com você.';
    _notification_link := '/dashboard/leads?tab=shared';
    
  ELSIF NEW.resource_type = 'lead' THEN
    SELECT l.title INTO _resource_name
    FROM leads l
    WHERE l.id = NEW.resource_id;
    
    _notification_title := _creator_name || ' compartilhou um negócio/lead';
    _notification_message := 'O lead "' || COALESCE(_resource_name, 'Sem nome') || '" foi compartilhado com você.';
    _notification_link := '/dashboard/leads?tab=shared';
    
  ELSE
    -- Default fallback
    _notification_title := _creator_name || ' compartilhou algo com você';
    _notification_message := 'Um recurso foi compartilhado com você.';
    _notification_link := '/dashboard';
  END IF;
