...
import { Plus, Trash2, FileText, Copy as CopyIcon } from 'lucide-react';
...
const BudgetsPage = () => {
  ...
  const saveBudget = () => {
    if (!client) return;
    setBudgets((prev) => [
      { id: Date.now().toString(), client, items, status: 'draft', createdAt: new Date().toLocaleDateString() },
      ...prev,
    ]);
    setCreating(false);
    setClient('');
          ...
          {budgets.map((b) => (
            <div key={b.id} className="flex items-center justify-between p-4 rounded-xl border border-border bg-card">
              <div>
                ...
                <p className="text-xs text-muted-foreground">{b.items.length} itens · {b.createdAt}</p>
              </div>
              <div className="flex items-center gap-3">
                <span className="font-semibold text-foreground">R$ {b.items.reduce((s, i) => s + i.quantity * i.unitPrice, 0).toFixed(2)}</span>
